...
void update_grid_multithreaded(char* src, char* dst, int n_threads) {
    ...
    if (!threads) {
        int n = grid_width*grid_height;         // thread for every cell

        // Use one thread for each tile
        n_threads = n_threads < 0 ? n : n_threads;

        threads = malloc(sizeof(pthread_t) * n_threads);
        _n_threads = n_threads;

        int block = n/n_threads + (n % n_threads ? 1 : 0);
        ...
        for (int i = 0; i < n; i += block) {
            ...
            args->end = (i+2*block > n) ? n : i+block;
